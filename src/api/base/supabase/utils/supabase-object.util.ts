...
const snakeToCamel = (str: string): string =>
  str
    .toLowerCase()
    .replace(/[-_][a-z0-9]/g, (group: string) => group.slice(-1).toUpperCase());

const camelToSnake = (str: string): string =>
  str.replace(/([A-Z0-9])/g, ($1: string) => `_${$1.toLowerCase()}`);
  ...
  const recurse = <T1 extends object, R1 extends object>(
    ...
    if (isPlainObject(input)) {
      return Object.keys(input).reduce(
        (acc, key) =>
          Object.assign(acc, {
            [options?.skipKeys?.includes(key)
              ? key
              : options?.map?.[key] || converter(key)]: recurse(input[key]),
          }),
        {} as R1
      );
